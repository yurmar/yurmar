...
const INITIAL_EMPLOYEES: Employee[] = [
    // Руководство
    { id: 1,  department: 'management', name: 'Иванов Дмитрий Сергеевич',   position: 'Генеральный директор',    phone: '+7 (495) 100-01-01', email: 'd.ivanov@digit-solutions.ru',    extension: '101' },
    ...
    // Отдел разработки
    { id: 4,  department: 'dev',        name: 'Новиков Алексей Иванович',    position: 'Руководитель отдела',     phone: '+7 (495) 100-02-01', email: 'a.novikov@digit-solutions.ru',   extension: '201' },
    { id: 5,  department: 'dev',        name: 'Морозова Анна Дмитриевна',    position: 'Senior Backend Developer',phone: '+7 (495) 100-02-02', email: 'a.morozova@digit-solutions.ru',  extension: '202' },
    { id: 6,  department: 'dev',        name: 'Волков Игорь Андреевич',      position: 'Senior Frontend Developer',phone: '+7 (495) 100-02-03', email: 'i.volkov@digit-solutions.ru',   extension: '203' },
    ...
    { id: 8,  department: 'dev',        name: 'Лебедев Роман Павлович',      position: 'Junior Developer',        phone: '+7 (495) 100-02-05', email: 'r.lebedev@digit-solutions.ru',   extension: '205' },
    // Отдел дизайна
    { id: 9,  department: 'design',     name: 'Попова Ирина Юрьевна',        position: 'Руководитель отдела',     phone: '+7 (495) 100-03-01', email: 'i.popova@digit-solutions.ru',    extension: '301' },
    ...
    { id: 15, department: 'support',    name: 'Захарова Юлия Игоревна',      position: 'Специалист поддержки',    phone: '+7 (495) 100-04-04', email: 'yu.zakharova@digit-solutions.ru', extension: '404' },
]
    ...
    return (
        <AnimatePresence>
            ...
            >
                ...
                <motion.div
                    className="relative z-10 w-full max-w-md bg-[#0d1a30] border border-white/10 rounded-2xl shadow-2xl p-6"
        ...
        >
            ...
            <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1 min-w-0">
                    <p className="font-semibold text-sm leading-tight line-clamp-2">{emp.name}</p>
...
function TableRow({ emp, onEdit, onDelete }: { emp: Employee; onEdit: () => void; onDelete: () => void }) {
    const dept = getDept(emp.department)
    return (
...
export default function PhoneBook() {
    ...
    const filtered = useMemo(() => {
        let list = employees
        if (activeDept !== 'all') list = list.filter(e => e.department === activeDept)
        if (search.trim()) {
    ...
    const openAdd = () => {
        setForm({ name: '', position: '', phone: '', extension: '', email: '', department: activeDept === 'all' ? 'management' : activeDept })
        setAddModal(true)
    }

    const openEdit = (emp: Employee) => {
        setTarget(emp)
        setForm({ name: emp.name, position: emp.position, phone: emp.phone, extension: emp.extension ?? '', email: emp.email, department: emp.department })
        setEditModal(true)
    }
    ...
    const handleAdd = () => {
        ...
        setEmployees(prev => [...prev, { id: nextId, ...form } as Employee])
        setAddModal(false)
    }

    const handleEdit = () => {
        if (!target || !form.name?.trim()) return
        setEmployees(prev => prev.map(e => e.id === target.id ? { ...e, ...form } as Employee : e))
        setEditModal(false)
    }
    ...
    const activeDeptData = activeDept === 'all' ? ALL_DEPT : (DEPARTMENTS.find(d => d.id === activeDept) ?? ALL_DEPT)
        ...
        <div className="min-h-screen pt-16 bg-[var(--background,#050d1f)]">
            ...
            <div className="sticky top-16 z-50 border-b border-white/8 bg-[#060e22]/95 backdrop-blur-xl">
                <div className="max-w-7xl mx-auto px-4 h-14 flex items-center gap-3">
                    ...
                    <button
                        onClick={openAdd}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium bg-sky-500/20 hover:bg-sky-500/30 text-sky-400 border border-sky-500/30 transition-colors flex-shrink-0"
                    >
                        <Plus size={13} /> <span className="hidden sm:inline">Добавить</span>
                    </button>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 py-6 flex gap-6">
                ...
                <aside className="hidden lg:block w-52 flex-shrink-0">
                    <div className="sticky top-32 space-y-0.5">
                        <p className="text-[10px] uppercase tracking-widest text-white/30 font-semibold px-3 mb-3 flex items-center gap-1.5">
                            <Building2 size={11} /> Отделы
                        </p>
                        {[ALL_DEPT, ...DEPARTMENTS].map(dept => (
                            <button
                                key={dept.id}
                            ...
                            >
                                ...
                                <span className={`ml-2 text-[10px] px-1.5 py-0.5 rounded-full font-mono tabular-nums flex-shrink-0 ${
                                    activeDept === dept.id ? 'bg-white/20 text-white' : 'bg-white/5 text-white/40'
                                }`}>
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="lg:hidden mb-4">
                        <button
                            ...
                            className={`flex items-center justify-between w-full px-4 py-2.5 rounded-xl bg-gradient-to-r ${activeDeptData.gradient} text-white text-sm font-medium`}
                        >
                            <span>{activeDeptData.name}</span>
                                ...
                                >
                                    {[ALL_DEPT, ...DEPARTMENTS].map(dept => (
                                        ...
                                        >
                                            ...
                                            <span className="text-xs text-white/30">{deptCounts[dept.id] ?? 0}</span>
                                        </button>
                                    ))}
                                </motion.div>
                            )}
                    ...
                    <div className="flex items-center justify-between mb-5">
                        <div>
                            <h2 className="font-semibold text-base text-white">{activeDeptData.name}</h2>
                            <p className="text-xs text-white/40 mt-0.5">
                                {search
                                    ? `Найдено: ${pluralEmployees(filtered.length)}`
                                    : pluralEmployees(filtered.length)
                                }
                            </p>
                        </div>
                        {search && (
                    ...
                    {filtered.length === 0 ? (
                        <div className="text-center py-24">
                            ...
                            <p className="text-white/30 text-sm">{search ? 'Ничего не найдено' : 'Нет сотрудников в этом отделе'}</p>
                            {!search && (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                            <AnimatePresence mode="popLayout">
                                {filtered.map(emp => (
                                    <EmployeeCard key={emp.id} emp={emp} onEdit={() => openEdit(emp)} onDelete={() => openDelete(emp)} />
                                ))}
                            </AnimatePresence>
                        </div>
                        ...
                        <div className="rounded-2xl overflow-hidden border border-white/8">
                            <table className="w-full">
                                ...
                                <tbody>
                                    <AnimatePresence>
                                        {filtered.map(emp => (
                                            <TableRow key={emp.id} emp={emp} onEdit={() => openEdit(emp)} onDelete={() => openDelete(emp)} />
                                        ))}
                                    </AnimatePresence>
                                </tbody>
                            </table>
